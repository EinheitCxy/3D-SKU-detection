...
**Goal:** Produce one isolated cold DA3-to-Three.js performance receipt for each of fd2–4 without modifying existing artifacts.

**Architecture:** A standard-library Python harness owns isolated output paths, stage subprocesses and `nvidia-smi` telemetry. Classification overlaps reconstruction/matching and joins before dedup; the reporter uses real case wall time so overlap is not double-counted. A separate browser runner uses Playwright because the existing frontend is native Three.js.
...
- Do not schedule warm cases or repeated browser navigations.
...
### Task 2: Capture isolated one-shot cold command and GPU evidence
...
- [ ] Start classification concurrently with reconstruction/matching and join it before analysis/dedup.
- [ ] Re-run focused tests and confirm they pass.

### Task 3: Measure one native Three.js navigation
...
- [ ] Write a failing test for one complete cache-disabled browser receipt.
...
- [ ] Implement summary JSON and Markdown rendering of one-shot cold means, shares and peak VRAM.
...
- [ ] Run resource detection, then one fd2 one-shot cold smoke.
- [ ] Run one serial cold case for each of fd2, fd3 and fd4 on GPU 2.
