...
内部工具 `code/utils/footprint_evidence.py` 会二次读取并验证 optional DA3 camera tensors，计算 source-world reconstruction residual、最多 512 个确定性采样点的双向 20 mm occlusion-aware reprojection，以及固定正式支撑平面的 per-ID leave-one-observation-out OBB diagnostics。双向 pair 只在不同 source image 之间生成；投影到 target 后，只有同时满足 observation valid mask、有效非零 world point、confidence 门和 finite positive depth 的 pixel 才进入 occluded/foreground/visible/mask-support 统计，其余单独计为 invalid target。该工具仅为 **shadow-only** internal consistency/sensitivity evidence：Task 5 尚未把它接入 `ground-stack-area`，Task 6 只会在正式 status/value/polygons/union/rejection reason 完全冻结后附加报告。camera fields 缺失或 malformed 只产生 `unavailable_*`/`failed_*` evidence，绝不会成为面积拒绝、硬门、置信区间或校准后的 m² accuracy claim。
