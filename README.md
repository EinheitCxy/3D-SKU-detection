...
核心 `src/web_viewer_export.py` 在几何过滤前读取 SAM3 mask，并通过 `protect_mask` 保留 mask 内的有效点，避免整件商品被小簇、离群或平面过滤删除。mask 内全部有效原始点也绕过体素合并与随机抽样，允许超过 150 万。mask 外继续过滤并降采样，独立使用固定最多 80 万点预算；商品点不占用背景预算。原始点云与 Surfel 共用结果。
...
背景点上限固定在核心导出器 `MAX_BACKGROUND_POINTS = 800_000`，不接受命令行或函数参数覆盖；SAM 内全部有效点继续保留。Viewer 按导出点和 `point_ranges` 渲染，不在浏览器二次删点。Docker 后端构建从 `CORE_REPO_ROOT/src` 复制同一核心实现，旧容器和旧 COS ZIP 需要更新后端并重新导出才生效。

## DA3 默认推理分辨率（2026-09-10）

生产默认长边为 **896**，由根目录 `da3_defaults.py` 统一定义；runner、生产 subprocess、匹配预处理及 pipeline 缓存复用使用相同设置。9:16 竖屏输入得到 **504×896**（宽×高），16:9 横屏得到 **896×504**。其他比例按原比例缩放，尺寸对齐到模型的 14 像素网格；不是强行拉伸到 16:9。

不同任务自动按输入像素宽高适配方向；同一任务混合横屏、竖屏或方形图片会在模型加载前报错，请按方向分别提交。相同方向、相同比例输入不裁剪。输入应使用与检测框一致的已正确朝向的图像像素；此改动不额外旋转照片或改变检测框坐标。

完整 pipeline 不再复用长边 504 或缺少预处理分辨率的缓存，会重新推理。独立 runner 仍可用 `--process_res` 做指定分辨率实验；`viewer-web` 仅导出已有缓存，不会升级旧包。已生成的本地/COS可视化需重新推理并导出，刷新网页不会提高深度网格。

896 的网格像素数约为旧 504 配置的 3.2 倍，推理显存与导出数据量会增加；保留既定的 SAM 全部有效点及最多 80 万背景点规则。

部署验证：`global-id-mapping:viewer-896-20260910` 已运行于 `global-id-mapping-local`，端口 8011；`/openapi.json` 返回 200，容器内 runner 与 matching 默认值均为 896。针对性检查 9 项通过（含真实 CPU 预处理）；未运行完整 GPU 视频推理、未覆盖已有 COS 包。旧容器保留为 `global-id-mapping-local-before-896-20260910`。
...
本地部署：`global-id-mapping:viewer-896-2m-quiet-20260910` 已替换 8011 服务。镜像入口检查确认 896 长边、商品 200 万/背景 80 万及 `quiet_outputs=True`；运行接口返回 HTTP 200。现有 COS 对象未覆盖。
