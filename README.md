...
内部工具 `code/utils/footprint_evidence.py` 会二次读取并验证 optional DA3 camera tensors，计算 source-world reconstruction residual、最多 512 个确定性采样点的双向 20 mm occlusion-aware reprojection，以及固定正式支撑平面的 per-ID leave-one-observation-out OBB diagnostics。该工具仅为 **shadow-only** internal consistency/sensitivity evidence：Task 5 尚未把它接入 `ground-stack-area`，Task 6 只会在正式 status/value/polygons/union/rejection reason 完全冻结后附加报告。camera fields 缺失或 malformed 只产生 `unavailable_*`/`failed_*` evidence，绝不会成为面积拒绝、硬门、置信区间或校准后的 m² accuracy claim。
