...
公开输出根为 `<save_root>/<dataset>/ground_stack_footprint/`。每次运行先把完整结果写入并 fsync 到不可变的 `runs/<run_id>/`，其中严格只有下列三个产物和 `manifest.json`；全部完成后才原子替换 `CURRENT`。读取方须先解析 `CURRENT`，再读取它指向的单一 generation：`CURRENT` replace 前的 write/fsync/rename/temp/replace 失败会向调用方报错并保留上一完整 generation 或不创建 CURRENT；成功 replace 是逻辑发布点，之后 output-root directory fsync 失败只产生明确的 durability warning，新 generation 仍作为成功结果返回且不尝试回滚。读取方始终不会读到跨运行混合的 JSON/GeoJSON/PNG；历史 generation 保持不变。
...
`ground-stack-area` 在全部 DA3/image/detection/mapping 合约验证通过后，使用公开缓存根 `<save_root>/<dataset>/sam3_mask_cache/v1/` 读写逐源帧不可变 mask bundle。key 覆盖 image ID 与原图 bytes/size、完整有序 object ID + 精确 binary64 bbox prompts、真实 checkpoint SHA-256、stage/cache/SAM3 code fingerprint、Python/NumPy/PyTorch/SAM3/CUDA/cuDNN/device/precision runtime fingerprint、完整 `predict_inst` contract 及 source-mask shape/dtype；任一项变化都会 miss 并重新计算。stage 在一次运行入口和全部 frame cache access 完成后各校验一次 checkpoint digest；真实 SAM3 miss 仍保留 producer 与 Task 3 model-load 边界的 before/after 校验。`measurement_report.json` 的 `sam3_mask_cache.frames[]` 按帧记录 key、payload/checkpoint digest、code provenance 与有序 `events`：正常冷启动为 `miss,written`，复用为 `hit`，损坏 bundle 会移入 `corrupt/` 后以 `invalid,written` 重算，非致命写失败为 `miss,cache_write_failed` 并仅使用当次完整 fresh masks。缓存命中永远不会提供 bbox fallback；空/无效 mask 仍触发整次 `rejected`/`value_m2: null`，也绝不发布 partial total。

`ground-stack-area` 现会在正式 status/value/plane/per-ID diagnostics/polygons/union/rejection reason 完全冻结后，调用内部 `code/utils/footprint_evidence.py` 附加 `evidence.mode=shadow` 报告。该工具二次读取并验证 optional DA3 camera tensors，计算 source-world reconstruction residual、最多 512 个确定性采样点的双向 occlusion-aware reprojection，以及固定正式支撑平面的 per-ID leave-one-observation-out OBB diagnostics。双向 pair 只在不同 source image 之间生成：投影点比 target depth 远超过 20 mm 记为 occluded（遮挡中性，不是冲突），近超过 20 mm 记为 foreground conflict，中间才是 visible-consistent；20 mm 只是 shadow diagnostic tolerance，绝不是正式面积门。只有一个 distinct source image 时明确标记 `single_observation_insufficient_cross_view_evidence`，不会冒充多视角证据。camera fields 缺失、malformed 或 evidence 计算失败只产生 `unavailable_*`/`failed_*`，不能改变正式面积或 generation geometry。该报告只是 internal consistency/sensitivity，不是 calibrated uncertainty、accuracy claim、confidence interval 或 error bound；任何 hard gate/error bound 都必须先在独立 calibration set 上完成校准与验证。
