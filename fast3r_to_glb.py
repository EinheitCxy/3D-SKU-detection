#!/usr/bin/env python3
"""
Fast3R GLB/GLTF点云导出工具
使用Fast3R进行3D重建并导出为GLB/GLTF格式
"""

import os
import sys
import torch
import numpy as np
import trimesh
from pathlib import Path
import argparse
from datetime import datetime

# 添加fast3r路径
sys.path.append(str(Path(__file__).parent / "fast3r"))

from fast3r.dust3r.inference_multiview import inference
from fast3r.dust3r.utils.image import load_images
from fast3r.dust3r.viz import pts3d_to_trimesh, cat_meshes
from fast3r.utils.checkpoint_utils import load_model


def extract_pointcloud_from_fast3r(output, min_conf_thr_percentile=30):
    """
    从Fast3R输出中提取点云数据
    
    Args:
        output: Fast3R推理结果
        min_conf_thr_percentile: 置信度阈值百分位数
        
    Returns:
        vertices: 3D点坐标 (N, 3)
        colors: 点云颜色 (N, 3)
    """
    preds = output['preds']
    views = output['views']
    
    all_points = []
    all_colors = []
    
    for i, (pred, view) in enumerate(zip(preds, views)):
        # 获取3D点和置信度
        pts3d = pred['pts3d']
        confidence = pred['conf']
        
        # 应用置信度阈值
        if min_conf_thr_percentile > 0:
            conf_thr = np.percentile(confidence.cpu().numpy(), min_conf_thr_percentile)
            mask = confidence > conf_thr
        else:
            mask = torch.ones_like(confidence, dtype=torch.bool)
        
        # 获取RGB图像
        img_rgb = view['img']
        if isinstance(img_rgb, torch.Tensor):
            img_rgb = img_rgb.cpu().numpy()
        
        # 规范化RGB值到[0,255]
        if img_rgb.max() <= 1.0:
            img_rgb = (img_rgb * 255).astype(np.uint8)
        elif img_rgb.max() <= 2.0:  # [-1,1] -> [0,255]
            img_rgb = ((img_rgb + 1) * 127.5).astype(np.uint8).clip(0, 255)
        
        # 应用mask并收集点云数据
        pts3d_np = pts3d.cpu().numpy()
        mask_np = mask.cpu().numpy()
        
        valid_points = pts3d_np[mask_np]
        valid_colors = img_rgb[mask_np]
        
        all_points.append(valid_points)
        all_colors.append(valid_colors)
    
    # 合并所有点云
    vertices = np.vstack(all_points)
    colors = np.vstack(all_colors)
    
    return vertices, colors


def create_mesh_from_fast3r(output, min_conf_thr_percentile=30):
    """
    从Fast3R输出创建三角网格
    
    Args:
        output: Fast3R推理结果
        min_conf_thr_percentile: 置信度阈值百分位数
        
    Returns:
        trimesh.Trimesh: 三角网格对象
    """
    preds = output['preds']
    views = output['views']
    
    meshes = []
    
    for i, (pred, view) in enumerate(zip(preds, views)):
        # 获取数据
        pts3d = pred['pts3d'].cpu().numpy()
        confidence = pred['conf'].cpu().numpy()
        img_rgb = view['img']
        
        if isinstance(img_rgb, torch.Tensor):
            img_rgb = img_rgb.cpu().numpy()
        
        # 规范化RGB值
        if img_rgb.max() <= 1.0:
            img_rgb = (img_rgb * 255).astype(np.uint8)
        elif img_rgb.max() <= 2.0:
            img_rgb = ((img_rgb + 1) * 127.5).astype(np.uint8).clip(0, 255)
        
        # 应用置信度阈值
        if min_conf_thr_percentile > 0:
            conf_thr = np.percentile(confidence, min_conf_thr_percentile)
            mask = confidence > conf_thr
        else:
            mask = np.ones_like(confidence, dtype=bool)
        
        # 生成mesh
        mesh_dict = pts3d_to_trimesh(img_rgb, pts3d, valid=mask)
        meshes.append(mesh_dict)
    
    # 合并所有mesh
    combined_mesh_dict = cat_meshes(meshes)
    combined_mesh = trimesh.Trimesh(**combined_mesh_dict)
    
    return combined_mesh


def export_pointcloud_to_glb(vertices, colors, output_path):
    """
    将点云数据导出为GLB格式
    
    Args:
        vertices: 3D点坐标 (N, 3)
        colors: 点云颜色 (N, 3)
        output_path: 输出文件路径
    """
    # 创建trimesh点云对象
    point_cloud = trimesh.PointCloud(vertices=vertices, colors=colors)
    
    # 导出为GLB格式
    point_cloud.export(output_path)
    print(f"点云已保存到: {output_path}")


def export_mesh_to_glb(mesh, output_path):
    """
    将三角网格导出为GLB格式
    
    Args:
        mesh: trimesh.Trimesh对象
        output_path: 输出文件路径
    """
    mesh.export(output_path)
    print(f"网格已保存到: {output_path}")


def main():
    parser = argparse.ArgumentParser(description='Fast3R GLB/GLTF导出工具')
    parser.add_argument('--images', type=str, required=True, 
                       help='输入图像目录路径')
    parser.add_argument('--model_path', type=str, 
                       default='naver/DUSt3R_ViTLarge_BaseDecoder_512_dpt',
                       help='模型路径或HuggingFace模型名称')
    parser.add_argument('--output_dir', type=str, default='./output',
                       help='输出目录')
    parser.add_argument('--min_conf_thr_percentile', type=int, default=30,
                       help='置信度阈值百分位数')
    parser.add_argument('--export_mesh', action='store_true',
                       help='导出三角网格而非点云')
    parser.add_argument('--device', type=str, default='cuda',
                       help='计算设备')
    
    args = parser.parse_args()
    
    # 创建输出目录
    os.makedirs(args.output_dir, exist_ok=True)
    
    print("=== Fast3R GLB/GLTF导出工具 ===")
    print(f"输入图像目录: {args.images}")
    print(f"输出目录: {args.output_dir}")
    print(f"使用设备: {args.device}")
    
    # 检查设备可用性
    if args.device == 'cuda' and not torch.cuda.is_available():
        print("CUDA不可用，切换到CPU")
        args.device = 'cpu'
    
    # 加载模型
    print("\n正在加载Fast3R模型...")
    model = load_model(args.model_path, args.device)
    print("模型加载完成")
    
    # 加载图像
    print(f"\n正在加载图像从: {args.images}")
    images = load_images(args.images, size=512)
    if len(images) == 0:
        print("错误: 未找到有效图像")
        return
    print(f"加载了 {len(images)} 张图像")
    
    # 进行3D重建
    print("\n正在进行3D重建...")
    output = inference(model, args.device, images, verbose=True)
    print("3D重建完成")
    
    # 生成时间戳
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    if args.export_mesh:
        # 导出三角网格
        print("\n正在生成三角网格...")
        mesh = create_mesh_from_fast3r(output, args.min_conf_thr_percentile)
        
        # 导出GLB格式
        glb_path = os.path.join(args.output_dir, f"fast3r_mesh_{timestamp}.glb")
        export_mesh_to_glb(mesh, glb_path)
        
        # 可选：同时导出GLTF格式
        gltf_path = os.path.join(args.output_dir, f"fast3r_mesh_{timestamp}.gltf")
        mesh.export(gltf_path)
        print(f"GLTF网格已保存到: {gltf_path}")
        
    else:
        # 导出点云
        print("\n正在提取点云数据...")
        vertices, colors = extract_pointcloud_from_fast3r(output, args.min_conf_thr_percentile)
        print(f"提取了 {len(vertices)} 个3D点")
        
        # 导出GLB格式
        glb_path = os.path.join(args.output_dir, f"fast3r_pointcloud_{timestamp}.glb")
        export_pointcloud_to_glb(vertices, colors, glb_path)
        
        # 可选：同时导出PLY格式
        ply_path = os.path.join(args.output_dir, f"fast3r_pointcloud_{timestamp}.ply")
        point_cloud = trimesh.PointCloud(vertices=vertices, colors=colors)
        point_cloud.export(ply_path)
        print(f"PLY点云已保存到: {ply_path}")
    
    print(f"\n=== 导出完成 ===")
    print(f"输出文件保存在: {args.output_dir}")


if __name__ == "__main__":
    main()