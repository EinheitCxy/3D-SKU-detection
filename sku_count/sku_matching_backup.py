...
class VGGTImageTransform:
    ...
    def apply_batch_padding(self, max_width: int, max_height: int) -> None:
        ...
        【修正】: VGGT 的批处理填充是左上角对齐 (top-left aligned)，
        而不是居中对齐。因此，padding 的偏移量始终为 0。
        这是导致 bbox 坐标不准的关键修正点。
        """
        # 修正：不再计算居中偏移，始终设置为0
        self.batch_pad_left = 0
        self.batch_pad_top = 0
...
def visualize_results(images: torch.Tensor, reference_idx: int, points_per_object: Dict[int, Dict], 
    ...
    """
    logger.info("Generating visualization results using VGGT coordinates...")
    ...
    try:
        ...
        for obj_id, data in points_per_object.items():
            ...
            if x1 < x2 and y1 < y2:  # 有效框
                ...
                # 绘制ID标签
                cv2.putText(overlay, f"ID: {obj_id}", (x1, max(y1 - 5, 10)), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
                
                logger.debug(f"✅ 参考图像绘制框 {obj_id}: ({x1}, {y1}, {x2}, {y2}) on {w}x{h}")
            else:
                logger.warning(f"⚠️ 参考图像框 {obj_id} 无效: ({x1}, {y1}, {x2}, {y2})")
        ...
        for s_idx, matched_boxes in correspondences.items():
                ...
                for bbox_info in target_bboxes:
                    ...
                    if x1 < x2 and y1 < y2:  # 有效框
                        ...
                        if target_bbox_id in matched_target_bbox_ids:
                            # 匹配的框：使用绿色粗线
                            cv2.rectangle(target_image_bgr, (x1, y1), (x2, y2), (0, 255, 0), 3)
                            cv2.putText(target_image_bgr, f"TARGET: {target_bbox_id} (MATCHED)", 
                                       (x1, max(y1 - 25, 15)), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 255, 0), 1)
                        else:
                            # 未匹配的框：使用灰色细线
                            cv2.rectangle(target_image_bgr, (x1, y1), (x2, y2), (128, 128, 128), 1)
                            cv2.putText(target_image_bgr, f"TARGET: {target_bbox_id} (UNMATCHED)", 
                                       (x1, max(y1 - 10, 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (128, 128, 128), 1)
            ...
            for item in matched_boxes:
                ...
                correspondence_ratio = item.get('correspondence_ratio', 0.0)
                matched_points = item.get('matched_points', 0)
                ...
                if x1 < x2 and y1 < y2:  # 有效框
                    # 绘制匹配框的引用ID和匹配信息（叠加在绿色框上方）
                    label = f"REF_ID: {obj_id} → T{target_bbox_id} ({correspondence_ratio:.2f}, {matched_points}/{total_points})"
                    ...
                    logger.debug(f"✅ 目标图像 {s_idx} 绘制匹配框 {obj_id}: ({x1}, {y1}, {x2}, {y2}) on {w}x{h}")
                else:
                    logger.warning(f"⚠️ 目标图像 {s_idx} 框 {obj_id} 无效: ({x1}, {y1}, {x2}, {y2})")
            ...
            logger.info(f"✅ Saved target image with all bboxes and matches to '{output_filename}'")
...
def match_objects_by_correspondence(
    ...
    for ref_object_id, ref_data in points_per_object.items():
        ...
        if best_match:
            ...
            logger.info(f"✅ Matched reference object {ref_object_id} with target bbox {best_match['target_bbox_id']} "
                       f"(ratio: {best_overlap_ratio:.3f}, points: {best_match['matched_points']}/{best_match['total_points']})")
        else:
    ...
    try:
        ...
        for s_idx in range(S):
            ...
            if matched_objects:
                ...
                logger.info(f"Found {len(matched_objects)} matched objects in image {s_idx}")
...
class SKUMatchingSystem:
    ...
    def process_images(self, image_folder: str, detection_dir: str, 
        ...
        try:
            ...
            logger.info(f"{len(image_paths)} images loaded")
            logger.info(f"Building resolution transformer...")
...
if __name__ == '__main__':
    ...
    try:
        ...
        image_folder = "../imdata/sample"
        detection_dir = "../imdata/detections_results_sample"
