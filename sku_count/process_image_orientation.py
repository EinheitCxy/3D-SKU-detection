...
def fix_image_orientation(image_path: str, output_path: str = None) -> bool:
    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            logger.info(f"Processing {Path(image_path).name}, EXIF orientation: {orientation}")
            ...
            if img_fixed.mode in ('RGBA', 'LA', 'P'):
                ...
                rgb_img.paste(img_fixed, mask=img_fixed.split()[-1] if img_fixed.mode in ('RGBA', 'LA') else None)
                img_fixed = rgb_img
...
def fix_directory_orientations(input_dir: str, output_dir: str = None, backup: bool = True) -> None:
    """批量修复目录中所有图像的方向
