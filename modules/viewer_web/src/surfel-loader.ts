...
export interface SurfelData {
  u: Uint16Array; v: Uint16Array; frames: Uint8Array;
  metadata: { version: number; point_count: number; grid_size: [number, number]; frames: SurfelFrame[] };
...
export async function loadSurfels(bundle: ViewerBundle, fetcher: typeof fetch = globalThis.fetch): Promise<SurfelData> {
  ...
  const [u, v, frames, depth] = await Promise.all([
  ...
  return { u, v, frames, metadata, textures, depths, textureSize: [textureWidth, textureHeight] };
}
