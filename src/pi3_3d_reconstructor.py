...
使用：
  uv run python -m src.pi3_3d_reconstructor --input_dir <images_dir> --output_file <out.glb>
"""
...
# 路径注入：确保可以导入 Pi3（若用户显式启用）和 root utils
THIS_DIR = Path(__file__).resolve().parent  # src/
REPO_ROOT = THIS_DIR.parent  # 3D_Recognization 或 3D_SKU_Detection
