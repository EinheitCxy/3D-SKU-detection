...
def load_images_as_tensor(path="data/truck", interval=1, PIXEL_LIMIT=255000, verbose=True):
    """
    Loads images from a directory, video, or list of paths, resizes them to a uniform size,
    then converts and stacks them into a single [N, 3, H, W] PyTorch tensor.

    Args:
        path: Can be:
            - str: directory path or video file path
            - list: list of image file paths (for frame-aligned image loading)
        interval: sampling interval (only used for directory/video)
        PIXEL_LIMIT: maximum number of pixels per image

    Returns:
        torch.Tensor: [N, 3, H, W] tensor
    """
    sources = []

    # --- 1. Load image paths or video frames ---
    if isinstance(path, list):
        # Handle list of image file paths (for frame-aligned image loading)
        if verbose:
            print(f"Loading {len(path)} images from provided list")
