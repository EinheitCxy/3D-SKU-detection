...
declare -A ALGO_CONFIGS=(
    ...
    ["da3"]="output_3dmapping_da3:accuracy_evaluation_da3:3D Mapping (DA3)"
)
...
if [ ! -f "$ACCURACY_SCRIPT" ]; then
    echo -e "${RED}错误: 准确性标注脚本不存在: $ACCURACY_SCRIPT${NC}"
    exit 1
fi
...
# 遍历所有算法类型
for algo_type in pt vggt pi3 da3; do
    echo ""
    echo "================================================================================"
    ...
    OUTPUT_BASE_DIR="$PROJECT_ROOT/Output/$FD/$output_subdir"
    RESULT_BASE_DIR="$PROJECT_ROOT/Output/$FD/$result_subdir"
    ...
    for ref_dir in "$OUTPUT_BASE_DIR"/*/; do
        if [ -d "$ref_dir" ]; then
            ...
            if [ -f "$matching_summary" ]; then
                ...
                if uv run python "$ACCURACY_SCRIPT" \
...
echo ""
echo "结果目录: $PROJECT_ROOT/Output/$FD/"
...
echo "  - accuracy_evaluation_da3/      (3D Mapping DA3)"
echo "================================================================================"
