...
# SKU匹配批量处理脚本
# 用法: ./batch_run_inference.sh [floor_display] [max_idx] [algorithm]
# 示例: ./batch_run_inference.sh floor_display2 4 point_tracking

# 参数设置
FLOOR_DISPLAY="${1:-floor_display2}"
MAX_IDX="${2:-4}"
ALGORITHM="${3:-point_tracking}"
...
OUTPUT_DIR="$PROJECT_ROOT/imdata/$FLOOR_DISPLAY"
...
for i in $(seq 0 $MAX_IDX); do
    ...
    uv run python modules/inference.py --algorithm "$ALGORITHM" --reference_idx $i --image_folder "$IMAGE_FOLDER" --detection_dir "$DETECTION_DIR" --output_dir "$OUTPUT_DIR"
