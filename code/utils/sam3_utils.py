...
API Modes:
  ...
  - batch inference API: Supports visual exemplars (positive/negative reference boxes).
    Use `sam3_masks_from_bboxes_batch()` for visual exemplar functionality.
"""
...
_SAM3_CACHE: Dict[Tuple[str, str], Tuple[object, object]] = {}

# Cache for batch inference API (model + transform + postprocessor)
_SAM3_BATCH_CACHE: Dict[Tuple[str, str, float], Tuple[object, object, object]] = {}
...
def _ensure_sam3_in_path() -> Path:
    ...
    sam3_repo = Path(__file__).resolve().parents[2] / "sam3"
...
def _get_sam3_batch_components(
    checkpoint_path: str, device: str, detection_threshold: float = 0.5, max_dets_per_img: int = -1
) -> Tuple[Any, Any, Any]:
    ...
    cache_key = (str(checkpoint_path), str(device), round(float(detection_threshold), 6), int(max_dets_per_img))
    if cache_key in _SAM3_BATCH_CACHE:
        return _SAM3_BATCH_CACHE[cache_key]
    ...
    # Standard validation transforms (resize to 1008x1008, normalize)
    ...
    _SAM3_BATCH_CACHE[cache_key] = (model, transform, postprocessor)
...
def mask_to_final_space(mask: np.ndarray, transform: ImageTransformBase) -> np.ndarray:
    ...
    logger.warning("mask_to_final_space: unsupported transform; returning original mask.")
    return mask_bool
...
def sam3_masks_from_bboxes_batch(
    image_path: str,
    bboxes_xyxy: List[List[float]],
    checkpoint_path: str,
    device: str,
    ...
    Examples:
        ...
        >>> masks = sam3_masks_from_bboxes_batch(
        ...     image_path="image.jpg",
...
def sam3_masks_with_exemplar(
    ...
    This is a simplified wrapper around sam3_masks_from_bboxes_batch for the
    ...
    Example:
        ...
        >>> masks = sam3_masks_with_exemplar(
        ...     image_path="image.jpg",
        ...     target_bboxes_xyxy=[],
        ...
        ... )
    """
    return sam3_masks_from_bboxes_batch(
...
def map_points_to_final(transform: ImageTransformBase, points_xy: np.ndarray) -> np.ndarray:
...
def sample_3d_points_from_mask(
    ...
    if mask_space == "final":
        ...
        pts_final = map_points_to_final(transform, candidates_2d)
