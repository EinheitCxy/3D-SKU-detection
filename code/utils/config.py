...
def build_matching_config_from_yaml(
    path: str | Path, algorithm: str | None = None, backend: str | None = None
) -> "SKUMatchingConfig":
    """Build SKUMatchingConfig from a YAML file.

    Args:
        path: YAML 配置路径
        algorithm: 可选算法覆盖（已收敛为单一 3D 映射算法，仅为向后兼容保留参数）
        backend: 可选后端覆盖，优先于 YAML 的 inference.backend（使 CLI --match_backend 生效 backend-aware 阈值）

    Expected schema (example):
    matching:
      algorithm: 3d_mapping  # 3D-2D 投影匹配（唯一算法）
      device: cuda
    ...
    Args:
        path: YAML path
        algorithm: optional override of matching.algorithm（仅保留向后兼容，忽略取值）
    Notes:
        - YAML 仅作为覆盖项（overrides）；未提供的字段使用 `SKUMatchingConfig.for_3d_mapping()` 的默认值。
    ...
    # backend 优先级：显式传入参数 > YAML override > 默认 da3
    backend = backend if backend is not None else overrides.pop("backend", "da3")
...
def extract_main_settings(data_or_path: Dict[str, Any] | str | Path) -> Dict[str, Any]:
    ...
    """
    data = (
        load_yaml_config(data_or_path)
        if not isinstance(data_or_path, dict)
        else data_or_path
    )
...
def extract_reconstruction_settings(
    data_or_path: Dict[str, Any] | str | Path,
) -> Dict[str, Any]:
...
class SKUMatchingConfig:
    ...
    sam3_use_self_exemplar: bool = (
        False  # 是否使用self-exemplar分割（每个bbox作为自己的visual exemplar）
    )
    sam3_self_exemplar_threshold: float = 0.5  # self-exemplar检测阈值
    sam3_device: str = "auto"  # SAM3单独设备: auto/cuda/cpu
    sam3_max_queries_per_forward: int = (
        16  # self-exemplar时每次forward的最大query数（降低显存峰值）
    )
    sam3_batch_image_size: int = (
        1008  # batch API resize尺寸（默认1008，显存紧张可降到768/640）
    )
    sam3_max_batch_size: int = (
        5  # self-exemplar每批最多bbox数(单ref一次forward的batch上限,大值=少forward overhead)
    )
    sam3_max_dets_per_query: int = (
        8  # batch API每个query保留的最大候选数（越小越省显存）
    )
    sam3_min_cuda_free_gb: float = (
        10.0  # sam3_device=auto 时启用 CUDA 的最小可用显存(GB)
    )

    # === 高斯分布采样参数 ===
    enable_gaussian_sampling: bool = (
        True  # 是否启用高斯分布采样（中心密集，向外正态递减）
    )
    gaussian_sigma: float = (
        0.3  # 高斯分布标准差（相对于bbox半径，0.2-0.4推荐，越小中心越集中）
    )
    gaussian_truncate: float = (
        3.0  # 高斯分布截断倍数（超过sigma*truncate的点权重接近0）
    )
    enable_gaussian_in_sam3_mask: bool = (
        True  # SAM3 mask内是否也应用高斯加权采样（混合采样模式）
    )
    ...
    min_3d_sample_points: int = (
        10  # 3D采样最少有效点数(不足则跳过该物体)，货架小bbox可调低
    )
    ...
    depth_consistency_threshold: float = (
        0.5  # 深度一致性阈值(米)，用于3D点采样端cache自洽性验证
    )
    plane_normal_alignment_threshold: float = (
        0.2  # 平面法向对齐阈值(|cos|夹角)，低于此值(夹角>78°)判非同面拒绝；放宽因环绕多视角下同平面法向估计有视角偏转
    )

    # === 3D验证性能优化参数 ===
    max_3d_validation_candidates: int = (
        5  # 最大3D验证候选框数量（预筛选Top-K，减少昂贵的3D采样）
    )
    ...
    def _validate_config(self):
        # Backend验证
        if self.backend != "da3":
            raise ValueError(f"backend must be 'da3', got {self.backend}")

        if self.max_points_per_bbox <= 0:
            raise ValueError(
                f"max_points_per_bbox must be positive, got {self.max_points_per_bbox}"
            )
        ...
        if self.min_confident_points > self.max_points_per_bbox:
            raise ValueError(
                f"min_confident_points ({self.min_confident_points}) cannot exceed max_points_per_bbox ({self.max_points_per_bbox})"
            )

        if self.min_bbox_area <= 0:
            raise ValueError(
                f"min_bbox_area must be positive, got {self.min_bbox_area}"
            )
        ...
        if self.enable_3d_mapping:
            if self.pairing_3d not in ("all", "next"):
                raise ValueError(
                    f"pairing_3d must be 'all' or 'next', got {self.pairing_3d}"
                )
            ...
            if not (0.0 <= self.projection_match_threshold <= 1.0):
                raise ValueError(
                    f"projection_match_threshold must be in [0,1], got {self.projection_match_threshold}"
                )

            if self.max_3d_distance <= 0:
                raise ValueError(
                    f"max_3d_distance must be positive, got {self.max_3d_distance}"
                )
        ...
        if self.min_non_overlap_area <= 0:
            raise ValueError(
                f"min_non_overlap_area must be positive, got {self.min_non_overlap_area}"
            )

        # 性能相关验证
        if self.max_total_points <= 0:
            raise ValueError(
                f"max_total_points must be positive, got {self.max_total_points}"
            )
        ...
        if self.max_total_points < self.max_points_per_bbox:
            ...
            warnings.warn(
                f"max_total_points ({self.max_total_points}) is less than max_points_per_bbox ({self.max_points_per_bbox}), may limit performance"
            )
    ...
    @classmethod
    def for_3d_mapping(cls, backend: str = "da3", **overrides) -> "SKUMatchingConfig":
        """创建3D映射算法的默认配置（仅覆盖算法特定参数，其他使用类默认值）

        Args:
            backend: 3D重建后端（仅 da3；metric 米制深度 + 原始 conf[1,6] 未归一化，需独立标定）
