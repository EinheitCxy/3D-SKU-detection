...
def visualize_results(
    ...
    Args:
        images: 图像张量 (S, C, H, W)，已经过模型预处理（等比例 resize 后的统一尺寸）
    ...
    try:
        ...
        ref_image_np = (
            images[reference_idx].permute(1, 2, 0).cpu().numpy() * 255
        ).astype(np.uint8)
        ...
        for obj_id, data in points_per_object.items():
            ...
            if x1 < x2 and y1 < y2:  # 有效框
                ...
                cv2.rectangle(
                    overlay, (x1, y1 - label_h - 10), (x1 + label_w, y1), color, -1
                )
                cv2.putText(
                    overlay,
                    label,
        ...
        for s_idx, matched_boxes in correspondences.items():
            # 使用模型输入空间的图像
            target_image_np = (
                images[s_idx].permute(1, 2, 0).cpu().numpy() * 255
            ).astype(np.uint8)
            ...
            if (
                detections
                and s_idx < len(detections)
                and transforms_info
                and s_idx < len(transforms_info)
            ):
                # 提取目标图像的所有检测框
                target_bboxes = extract_bboxes_from_detections(
                    [detections[s_idx]], 0, config
                )
                ...
                for item in matched_boxes:
                    matched_target_bbox_ids.add(
                        item.get("target_obj_id", item.get("target_bbox_id"))
                    )
                ...
                for bbox_info in target_bboxes:
                    ...
                    if x1 < x2 and y1 < y2:  # 有效框
                        ...
                        if target_bbox_id in matched_target_bbox_ids:
                            ...
                            color = (0, 255, 0)
                            cv2.rectangle(
                                target_image_bgr, (x1, y1), (x2, y2), color, 2
                            )

                            # 绘制中心点
                            center_x, center_y = (x1 + x2) // 2, (y1 + y2) // 2
                            cv2.circle(
                                target_image_bgr, (center_x, center_y), 3, color, -1
                            )
                            ...
                            color = (128, 128, 128)
                            cv2.rectangle(
                                target_image_bgr, (x1, y1), (x2, y2), color, 1
                            )  # 细线
            ...
            for item in matched_boxes:
                ...
                # 获取模型输入坐标（而不是原图坐标）
                input_box = item.get("model_box", [])
                target_bbox_id = item.get(
                    "target_obj_id", item.get("target_bbox_id", "N/A")
                )

                if not input_box or len(input_box) != 4:
                    logger.warning(
                        f"WARNING: 目标图像 {s_idx} 对象 {obj_id}: 无效的模型输入坐标 {input_box}"
                    )
                    continue
            ...
            output_filename = (
                ...
                / f"target_image_{s_idx}_all_bboxes_and_matches.jpg"
            )
        ...
        with open(summary_path, "w", encoding="utf-8") as f:
            ...
            for target_idx in sorted(correspondences.keys()):
                ...
                for match in sorted_matches:
                    ...
                    target_id = match.get(
                        "target_obj_id", match.get("target_bbox_id", "N/A")
                    )
                    ...
                    f.write(
                        f"Matched ref {ref_id} → target {target_id} (hit ratio: {ratio:.2f} {matched_points}/{total_points})\n"
                    )

                # 写入分组信息（使用真实的参考图像索引）
                f.write(
                    f"Matching objects between reference image {reference_image_idx} and target image {target_idx}\n"
                )
            ...
            # 写入汇总信息
            f.write(
                f"{algo_name} complete. Found correspondences in {len(correspondences)} images.\n"
            )
            f.write(
