...
from .data_utils import load_detections, save_correspondences_json
...
# 跨 ref 复用避免每 ref 重复 PIL open+resize+to_tensor）。
...
class SKUMatchingSystem:
    ...
    def initialize(self) -> None:
        ...
        logger.info(
            f"Initializing SKU matching system (backend: {self.config.backend})..."
        )
        ...
        try:
            ...
            # DA3 后端：不需要加载模型（直接从缓存加载预重建数据）
            logger.info(
                f"{self.config.backend.upper()} 后端：跳过模型加载，将从 {self.config.backend}_cache 加载预重建数据"
            )
    ...
    def process_images(
        ...
        try:
            ...
            image_paths, detections = self._load_data(
                image_folder, detection_dir, max_images
            )
            ...
            logger.info(
                f"Loaded {len(image_paths)} images with {len(detections)} detection files"
            )
            ...
            # images resize 到 transforms target 尺寸（float [0,1]，与 ToTensor 格式一致）
    ...
    def _load_data(
        self, image_folder: str, detection_dir: str, max_images: Optional[int]
        ...
        logger.info(
            f"Found {len(image_files)} images and {len(detections)} detection files"
        )
        logger.info(f"Image IDs: {image_ids}")
        logger.info(f"Detection IDs: {detection_indices}")

        # 3) 使用 ReconstructionDetectionAligner 进行帧对齐验证
        # 构建占位 reconstruction_data（仅用于对齐验证，不需要真实的world_points）
        import numpy as np
        ...
        _, aligned_detections, alignment_report = (
        ...
        aligned_image_ids = alignment_report.get(
            "repaired_image_ids", alignment_report["common_ids"]
        )
        ...
        if alignment_report.get("repair_applied"):
            logger.info(
                f"Alignment repaired: {alignment_report['repaired_frame_count']} frames aligned"
            )
            logger.info(
                f"   Dropped images: {alignment_report.get('dropped_frames', 0)}"
            )
            logger.info(
                f"   Dropped detections: {alignment_report.get('dropped_detection_frames', 0)}"
            )
        # else:
    ...
    def _run_matching(
        ...
        use_amp = (
            ...
            and (
                isinstance(self.config.device, str)
        ...
        if self.config.enable_3d_mapping:
            ...
            if pairing == "next":
                ...
                if total_images >= 2:
                    ...
                    target_indices = []
                    logger.info(
                        "3D pairing=next: only 1 image available, no targets to match"
                    )

        amp_ctx = (
            torch.amp.autocast("cuda", dtype=self.config.dtype)
            if use_amp
            else nullcontext()
        )
        ...
        try:
            ...
            if "out of memory" in str(e).lower() and torch.cuda.is_available():
                logger.error(
                    f"CUDA out of memory during {algorithm_name}. Try reducing max_images or max_points_per_bbox"
                )
        ...
        logger.info(
            f"Found {total_matches} matches across {len(correspondences)} images"
        )
    ...
    def _post_process_results(
        ...
        if self.config.save_json and correspondences:
            ...
            for target_idx, objs in correspondences.items():
                ...
                if match_count > 0:
                    avg_hit = (
                        sum(o.get("correspondence_ratio", 0.0) for o in objs)
                        / match_count
                    )
                    avg_match_frac = (
                        sum(
                            (
                                float(o.get("matched_points", 0))
                                / max(1.0, float(o.get("total_points", 1)))
                            )
                            for o in objs
                        )
                        / match_count
            ...
            save_correspondences_json(
                correspondences, points_per_object, self.config, meta
            )

        if not correspondences:
            logger.warning(
                f"No object correspondences found for reference image {reference_image_idx}"
            )
