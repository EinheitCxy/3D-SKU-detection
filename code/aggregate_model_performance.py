...
def parse_summary_file(file_path: Path) -> Dict:
    ...
    # 提取所有参考图片的指标
    metrics = []
    pattern = r"总体召回率 \(Recall\): ([\d.]+)%.*?模型有效率 \(Effectiveness\): ([\d.]+)%.*?Reference ID映射准确率 \(Precision\): ([\d.]+)%"
    ...
    for match in re.finditer(pattern, content, re.DOTALL):
        ...
        metrics.append(
            {"recall": recall, "effectiveness": effectiveness, "precision": precision}
        )
...
def main():
    ...
    # 算法名称映射
    algo_mapping = {
        "accuracy_evaluation_da3": "3D Mapping (DA3)",
    }
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for dataset_name in sorted(all_data.keys()):
            ...
            for algo_name in sorted(all_data[dataset_name].keys()):
                ...
                f.write(
                    f"  - 平均有效率 (Effectiveness): {metrics['avg_effectiveness']:.2f}%\n"
                )
                f.write(
                    f"  - 平均准确率 (Precision): {metrics['avg_precision']:.2f}%\n"
                )
        ...
        # 按算法汇总（加权平均）
        algo_aggregated = defaultdict(
            lambda: {
                "recall_weighted": 0,
                "effectiveness_weighted": 0,
                "precision_weighted": 0,
                "recall_simple": [],
                "effectiveness_simple": [],
                "precision_simple": [],
                "count": 0,
                "dataset_count": 0,
            }
        ...
        for dataset_name, algos in all_data.items():
            for algo_name, metrics in algos.items():
                ...
                algo_aggregated[algo_name]["recall_weighted"] += (
                    metrics["avg_recall"] * weight
                )
                algo_aggregated[algo_name]["effectiveness_weighted"] += (
                    metrics["avg_effectiveness"] * weight
                )
                algo_aggregated[algo_name]["precision_weighted"] += (
                    metrics["avg_precision"] * weight
                )
                algo_aggregated[algo_name]["recall_simple"].append(
                    metrics["avg_recall"]
                )
                algo_aggregated[algo_name]["effectiveness_simple"].append(
                    metrics["avg_effectiveness"]
                )
                algo_aggregated[algo_name]["precision_simple"].append(
        ...
        for algo_name in sorted(algo_aggregated.keys()):
            ...
            weighted_recall = (
                data["recall_weighted"] / total_count if total_count > 0 else 0
            )
            weighted_eff = (
                data["effectiveness_weighted"] / total_count if total_count > 0 else 0
            )
            weighted_prec = (
                data["precision_weighted"] / total_count if total_count > 0 else 0
            )
        ...
        recall_ranking = sorted(
            algo_aggregated.items(),
            key=lambda x: (
                x[1]["recall_weighted"] / x[1]["count"] if x[1]["count"] > 0 else 0
            ),
            reverse=True,
        )
        ...
        for rank, (algo_name, data) in enumerate(recall_ranking, 1):
            weighted_recall = (
                data["recall_weighted"] / data["count"] if data["count"] > 0 else 0
            )
        ...
        effectiveness_ranking = sorted(
            algo_aggregated.items(),
            key=lambda x: (
                x[1]["effectiveness_weighted"] / x[1]["count"]
                if x[1]["count"] > 0
                else 0
            ),
            reverse=True,
        ...
        for rank, (algo_name, data) in enumerate(effectiveness_ranking, 1):
            weighted_eff = (
                data["effectiveness_weighted"] / data["count"]
                if data["count"] > 0
                else 0
            )
        ...
        precision_ranking = sorted(
            algo_aggregated.items(),
            key=lambda x: (
                x[1]["precision_weighted"] / x[1]["count"] if x[1]["count"] > 0 else 0
            ),
            reverse=True,
        )
        ...
        for rank, (algo_name, data) in enumerate(precision_ranking, 1):
            weighted_prec = (
                data["precision_weighted"] / data["count"] if data["count"] > 0 else 0
            )
