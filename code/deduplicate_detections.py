"""
根据匹配关系对检出框去重的实用脚本
uv run deduplicate_detections.py --dataset imdata0911/floor_display2
...
输出
- 将去重后的 JSON 写入 sku_count/output_dedup/<dataset_name>/<image_idx>_dedup.json
"""
...
def deduplicate_for_images(paths: DatasetPaths,
    ...
    out_dir = (output_root or (Path(__file__).parent / 'output_dedup')) / dataset_name
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default=None, help='输出根目录（默认: sku_count/output_dedup/<dataset_name>）')
