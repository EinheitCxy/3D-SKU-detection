...
#   [2] SKU检测 -> detections_results/0.json,1.json,...（根目录 bbox_gen.py）
...
DETECTOR_ROOT="${DETECTOR_ROOT:-$REPO_DIR}"
DETECTOR_ENV="${DETECTOR_ENV:-$DETECTOR_ROOT/.venv}"
CODE_ENV="${CODE_ENV:-$CODE_DIR/.venv}"
...
[ -x "$CODE_ENV/bin/python" ] || { echo "[ERROR] 核心环境不存在: $CODE_ENV" >&2; exit 1; }
echo "=== video -> 去重结果 ==="
echo "  video      : $VIDEO"
...
run_frame_sample() {
  ...
  cd "$CODE_DIR"
  run_python "$CODE_ENV" - "$VIDEO" "$DATASET_DIR/images" "$FPS" <<'PYEOF'
import sys, cv2, os
...
run_pipeline() {
  ...
  cd "$CODE_DIR"
  nvidia-smi -L >/dev/null 2>&1 || { echo "[ERROR] Pi3 匹配需要可用 NVIDIA GPU；检测结果已保留，未生成 global_mapping.json。" >&2; exit 2; }
  run_python "$CODE_ENV" main.py --mode pipeline --dataset "$DATASET_DIR" \
...
extract_results() {
  ...
  cd "$CODE_DIR"
  local n; n=$(run_python "$CODE_ENV" -c "import json; print(len(json.load(open('$GM'))))")
  echo "  ============================================"
