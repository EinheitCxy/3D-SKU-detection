...
def extract_metrics_from_file(file_path: str) -> Optional[Dict[str, float]]:
    ...
    try:
        ...
        # 提取模型有效率 (Effectiveness): 52.83% (28/53)
        effectiveness_pattern = r"模型有效率.*?(\d+\.?\d*)%.*?\((\d+)/(\d+)\)"
...
def calculate_weighted_averages(
    ...
    for file_path, metrics in filtered_metrics:
        ...
        if all(
            key in metrics
            for key in ["effectiveness", "effectiveness_den", "effectiveness_num"]
        ):
        ...
        # 精确率加权计算
        if all(
            key in metrics for key in ["precision", "precision_den", "precision_num"]
        ):
    ...
    if recall_total_weight > 0:
        ...
        result["recall_overall"] = (
            (recall_total_num / recall_total_den * 100) if recall_total_den > 0 else 0
        )
        result["recall_total_correct"] = recall_total_num
        result["recall_total_samples"] = recall_total_den

    if effectiveness_total_weight > 0:
        result["effectiveness_weighted_avg"] = (
            effectiveness_weighted_sum / effectiveness_total_weight
        )
        result["effectiveness_overall"] = (
            (effectiveness_total_num / effectiveness_total_den * 100)
            if effectiveness_total_den > 0
            else 0
        )
    ...
    if precision_total_weight > 0:
        result["precision_weighted_avg"] = (
            precision_weighted_sum / precision_total_weight
        )
        result["precision_overall"] = (
            (precision_total_num / precision_total_den * 100)
            if precision_total_den > 0
            else 0
        )
...
def main():
    ...
    for txt_file in all_txt_files:
        ...
        if metrics:
            ...
            print(
                f"   召回率: {metrics['recall']:.2f}%, 有效率: {metrics['effectiveness']:.2f}%, 精确率: {metrics['precision']:.2f}%"
            )
        else:
    ...
    if "recall_weighted_avg" in weighted_averages:
        ...
        print(
            f"  - 整体计算: {weighted_averages['recall_overall']:.2f}% ({weighted_averages['recall_total_correct']}/{weighted_averages['recall_total_samples']})"
        )
    ...
    if "effectiveness_weighted_avg" in weighted_averages:
        ...
        print(
            f"  - 整体计算: {weighted_averages['effectiveness_overall']:.2f}% ({weighted_averages['effectiveness_total_correct']}/{weighted_averages['effectiveness_total_samples']})"
        )
    ...
    if "precision_weighted_avg" in weighted_averages:
        ...
        print(
            f"  - 整体计算: {weighted_averages['precision_overall']:.2f}% ({weighted_averages['precision_total_correct']}/{weighted_averages['precision_total_samples']})"
        )
