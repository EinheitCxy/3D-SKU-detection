...
def main():
    ...
    parser.add_argument(
        "--image_dir",
        type=str,
        default="../imdata/floor_display3/images",
        help="图片目录路径",
    )
    parser.add_argument(
        "--detection_dir",
        type=str,
        default="../imdata/floor_display3/detections_results",
        help="检测结果目录路径",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../imdata/floor_display3/imdata_with_bbox",
        help="输出目录路径 ",
    )
    parser.add_argument(
        "--confidence_threshold", type=float, default=0.3, help="置信度阈值"
    )
    parser.add_argument("--no_confidence", action="store_true", help="不显示置信度信息")
    parser.add_argument("--no_class", action="store_true", help="不显示类别信息")
    parser.add_argument(
        "--image_format", type=str, default="jpg", help="图片格式 (default: jpg)"
    )
    ...
    for num, image_path in numeric_images:
        ...
        for bx in bboxes:
            ...
            draw_bbox_with_id(
                image, bx["bbox"], bx["object_id"], color, thickness=2, font_scale=0.6
            )
