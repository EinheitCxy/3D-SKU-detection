...
try:
    ...
    from utils import SKUMatchingConfig, SKUMatchingSystem
    from utils.profiling import StageTimer, set_enabled
except ImportError as e:
    logger.error(f"模块导入错误: {e}")
    logger.error("请确保 utils 模块可正常导入")
    sys.exit(1)


def _compute_output_dir(
    base: str, algorithm_type: str, ref_idx: int, backend: str | None = None
) -> str:
    """根据算法类型和后端计算输出目录。
...
def _count_images_and_detections(
    image_folder: str, detection_dir: str
) -> tuple[int, int]:
...
def create_config_from_args(
    args, algorithm_type: str = "3d_mapping"
) -> SKUMatchingConfig:
    ...
    Args:
        ...
        algorithm_type: 算法类型，'3d_mapping'，决定使用哪个预设配置模板
...
def _create_config_from_yaml(args, algorithm_type: str) -> SKUMatchingConfig:
    ...
    cfg = build_matching_config_from_yaml(
        args.config, algorithm=algorithm_type, backend=getattr(args, "backend", None)
    )
    # Always route outputs to per-ref subdir like CLI path does
    cfg.output_dir = _compute_output_dir(
        args.output_dir,
        algorithm_type,
        args.reference_idx,
        getattr(args, "backend", None),
...
def run_3d_mapping_algorithm(args) -> dict:
    ...
    num_images, num_dets = _count_images_and_detections(
        args.image_folder, args.detection_dir
    )
    logger.info(
...
def main() -> None:
    ...
    解析命令行参数，验证输入路径，并执行 3D-2D投影匹配算法。
    ...
    Example:
        基本使用:
        >>> python inference.py --algorithm 3d
    ...
    parser.add_argument(
        "--detection_dir",
        type=str,
        default="../imdata/floor_display2/detections_results",
        help="检测结果目录路径",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="../imdata/floor_display2",
        help="输出根目录路径",
    )
    ...
    # 系统参数
    parser.add_argument(
        "--device", type=str, default="cuda", help="计算设备 (cuda/cpu)"
    )
    parser.add_argument("--seed", type=int, default=42, help="随机种子")
    parser.add_argument("--save_json", action="store_true", help="保存结果为JSON文件")
    # 匹配参数
    parser.add_argument(
        "--max_points_per_bbox", type=int, default=30, help="每个检测框最大采样点数"
    )
    parser.add_argument(
        "--confidence_threshold", type=float, default=0.0, help="点追踪置信度阈值"
    )
    parser.add_argument(
        "--min_confident_points", type=int, default=10, help="每个bbox的最小置信点数"
    )
    parser.add_argument(
        "--min_hit_ratio", type=float, default=0.5, help="最小命中率阈值"
    )
