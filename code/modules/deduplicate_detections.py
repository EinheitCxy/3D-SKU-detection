...
- 解析匹配产出的 matching_summary.txt（参考 0-based → 目标 0-based 的对应关系）
...
def resolve_dataset_paths(dataset_dir: Path) -> DatasetPaths:
    ...
    summary_file = dataset_dir / "output_3dmapping_da3" / "0" / "matching_summary.txt"
    return DatasetPaths(
...
def save_detection_objects(
    json_path: Path, original: Dict, new_objects: List[Dict]
) -> None:
    ...
    if (
        ...
        and "skus" in original
        and isinstance(original["skus"], list)
        and original["skus"]
    ):
        # 从skus[0]中提取classes
        data = {
...
def save_merged_detections_with_gid(
    json_path: Path,
    ...
    logger.info(
        f"Saved merged detections with global_id to: {json_path} ({len(merged_output)} images)"
    )
...
def parse_all_matches(summary_root: Path) -> List[Dict]:
    ...
    re_match = re.compile(
        r"Matched ref\s+(\d+)\s*.*?target\s+(\d+)\s+\(hit ratio:\s+([\d.]+)\s+(\d+)/(\d+)\)"
    )
    ...
    for p in sorted(summary_root.iterdir(), key=lambda x: x.name):
        ...
        pending: List[Tuple[int, int, float, int, int]] = (
            []
        )  # (ref_id, target_id, hit_ratio, mp, tp)
        ...
        for line in lines:
            ...
            if f:
                ...
                if len(pending) < n:
                    logger.warning(
                        f"[{sf}] Found {n} but only {len(pending)} pending matches; assigning all pending"
                    )
                    n = len(pending)
...
def filter_best_matches(matches: List[Dict]) -> List[Dict]:
    ...
    for (ref_idx, target_idx), pair_matches in pair_groups.items():
        ...
        sorted_matches = sorted(
            pair_matches, key=lambda x: x["hit_ratio"], reverse=True
        )
...
def _list_numeric_detection_indices(detections_dir: Path) -> List[int]:
    ...
    try:
        detections_with_index = load_detections(
            str(detections_dir), return_index_map=True
        )
...
def deduplicate_sequence(
    ...
    Args:
        ...
        algorithm: 算法类型 '3d_mapping'/'3d'，决定匹配结果目录名
        backend: 3D算法后端 'da3'，仅在algorithm='3d_mapping'时生效
    """
    ...
    if all_matches:
        ...
        cov0 = sum(
            ...
            if int(m.get("ref_idx", 0)) in det_set
            and int(m.get("target_idx", 0)) in det_set
        )
        cov1 = sum(
            1
            for m in all_matches
            if (int(m.get("ref_idx", 0)) + 1) in det_set
            and (int(m.get("target_idx", 0)) + 1) in det_set
        )
        offset = 1 if cov1 > cov0 else 0
        if cov1 == cov0:
            min_det = min(det_set)
            min_match = min(
                min(int(m.get("ref_idx", 0)), int(m.get("target_idx", 0)))
                for m in all_matches
            )
        ...
        if offset:
            all_matches = [
                {
                    **m,
                    "ref_idx": int(m.get("ref_idx", 0)) + 1,
                    "target_idx": int(m.get("target_idx", 0)) + 1,
                }
                for m in all_matches
            ]
        logger.info(
            f"[DEBUG] index_offset={offset} (coverage offset0={cov0}, offset1={cov1})"
        )

    if min_hit_ratio > 0 and all_matches:
        all_matches = [
            m for m in all_matches if float(m.get("hit_ratio", 0.0)) >= min_hit_ratio
        ]
    ...
    if dedup_mode == "best":
        ...
        matches_for_gid: List[Dict] = (
            filter_best_matches(all_matches) if all_matches else []
        )

    # === 调试：检查去重和全局ID使用的匹配数量差异 ===
    logger.info(
        f"[DEBUG] 去重使用的匹配数(matches_for_dedup): {len(matches_for_dedup)}"
    )
    ...
    if not indices or indices[0] != 1:
        logger.warning(
            "Detections do not start at 1; proceeding with available indices"
        )
    ...
    for i in indices:
        ...
        if is_first_image:
            ...
            new_objects = [
                obj for idx, obj in enumerate(objects) if idx not in drop_ids
            ]
            survivors_by_image[i] = set(
                idx for idx in range(len(objects)) if idx not in drop_ids
            )
            objects_by_image[i] = objects
            logger.debug(
    ...
    logger.info(
        f"Sequence dedup finished for images {indices[0]}..{indices[-1]} (total {len(indices)})"
    )
    ...
    try:
        ...
        json_strings = add_global_id_to_jsons(
            detections_dir=paths.detections_dir, global_mapping=mapping, indices=indices
        )
        ...
        logger.info(
            f"Saved global SKUs with metadata to: {global_skus_path} ({len(json_strings)} images)"
        )
...
def build_global_mapping(
    ...
    matches = sorted(
        matches, key=lambda m: float(m.get("hit_ratio", 0.0)), reverse=True
    )
    ...
    for img_id in image_indices:
        ...
        logger.info(
            f"[DEBUG] 图片{img_id}: 原始物体={len(objs)}, 保留物体={img_survivor_count}"
        )
    ...
    logger.info(
        f"[DEBUG] 其中: 保留物体节点={survivor_nodes}, 仅匹配节点(被删除)={match_only_nodes}"
    )
    ...
    # 统计有多少连通分量包含多个保留物体
    multi_survivor_components = [
        (root, nodes) for root, nodes in root_to_survivors.items() if len(nodes) > 1
    ]
    logger.info(
        f"[DEBUG] 包含多个保留物体的连通分量数: {len(multi_survivor_components)}"
    )
        ...
        for m in matches:
            ...
            if r_node in component_nodes and t_node in component_nodes:
                relevant_matches.append(
                    f"({r_node[0]},{r_node[1]})->({t_node[0]},{t_node[1]})"
                )
...
def add_global_id_to_jsons(
    detections_dir: Path, global_mapping: Dict[str, List[Dict]], indices: List[int]
) -> List[str]:
    ...
    for i in indices:
        ...
        # 统计被去重的对象数量
        dedup_count = sum(
            1 for obj in objects_with_metadata if obj.get("is_deduplicated", False)
        )
        logger.info(
            f"Image {i}: processed {len(objects_with_metadata)} objects "
...
def main():
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default=str(Path("imdata0911") / "floor_display2"),
        help="数据集根目录",
    )
    parser.add_argument(
        "--summary",
        type=str,
        default=None,
        help="matching_summary.txt 路径（默认: <dataset>/output_3dmapping_da3/0/matching_summary.txt）",
    )
    parser.add_argument(
        "--max_image", type=int, default=None, help="处理到最大图片编号(含)"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="输出根目录（默认: output_dedup/<dataset_name>）",
    )
    parser.add_argument(
        "--same_names",
        action="store_true",
        help="输出文件名与原始一致（1.json, 2.json, ...），而不是 *_dedup.json",
    )
    parser.add_argument(
        "--dedup_mode",
        type=str,
    ...
    parser.add_argument(
        "--algorithm",
        type=str,
        choices=["3d_mapping", "3d"],
        default="3d_mapping",
        help="算法类型：3d_mapping=3D映射（默认 3d_mapping）",
    )
    parser.add_argument(
        "--backend",
        type=str,
        choices=["da3"],
        default="da3",
        help="3D算法后端：da3（仅在algorithm=3d_mapping时生效）",
    )
