...
class SKUDetectionMain:
    ...
    def run_improved_sku_analysis(self, dataset_path: str, algorithm: str = 'point_tracking', backend: str | None = None) -> StepResult:
        ...
        try:
            ...
            # 检测使用的算法
            algorithm_name = "Point Tracking" if "output_pt" in str(summary_dir) else "3D Mapping"
    ...
    def run_complete_pipeline(self, dataset_path: str, algorithm: str = 'point_tracking', model_path: str | None = None) -> Dict[str, bool]:
        ...
        # 3. SKU计数分析
        analysis = self.run_improved_sku_analysis(dataset_path, algorithm=algorithm, backend=match_backend)
...
def main() -> None:
    ...
    if args.mode == 'interactive':
        ...
        app.run_improved_sku_analysis(args.dataset, algorithm=args.algorithm, backend=analyzer_backend)
    elif args.mode == 'dedup':
        # dedup 模式需要明确的单一算法类型
        if args.algorithm == 'both':
            logger.error("dedup 模式不支持 algorithm='both'，请指定 'point_tracking' 或 '3d'")
            sys.exit(1)
        # 根据算法类型选择后端
