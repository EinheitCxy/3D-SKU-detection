...
class SKUDetectionMain:
    ...
    def run_sku_matching(
        ...
        *,
        ...
        - backend: 3D重建模型后端，用于3D算法时选择数据源
        - parallel_refs: 并行处理的参考图片数（>1 时启用线程池；推荐 da3 后端使用）
    ...
    def run_detection_visualization(
        ...
        try:
            ...
            output_viz_dir = (
                ...
                if self.save_root
                else (CODE_DIR / dataset.name / output_suffix)
            ).resolve()
    ...
    def run_improved_sku_analysis(
        self, dataset_path: str, algorithm: str = "3d", backend: str | None = None
    ) -> StepResult:
        ...
        try:
            ...
            reports_dir = (
                ...
                if self.save_root
                else CODE_DIR / dataset.name / "output_reports"
            )
    ...
    def run_accuracy_evaluation(self, dataset_path: str) -> StepResult:
        ...
        try:
            ...
            dataset = Path(dataset_path)
            match_out_dir = (
                (self.save_root / dataset.name / "output_3dmapping_da3")
                if self.save_root
                else (dataset / "output_3dmapping_da3")
            )
            if not match_out_dir.exists():
    ...
    def run_reconstruction(
        ...
        *,
        ...
        """生成3D点云/GLB（支持后端：已注册的 ReconstructorBase 子类，当前 da3）。
        ...
        try:
            ...
            if use_backend not in RECONSTRUCTOR_REGISTRY:
                ...
                raise ValueError(
                    f"未知/未启用的重建后端: {backend}. 已注册: {available}"
                )
    ...
    def run_dedup_sequence(
        ...
        Args:
            dataset_path: 数据集路径
            algorithm: 算法类型 '3d'/'3d_mapping'
            backend: 3D算法后端，仅在algorithm='3d_mapping'时生效
        """
        start = perf_counter()
        try:
            logger.info(f"开始顺序去重 (algorithm: {algorithm}, backend: {backend})")
            from modules.deduplicate_detections import (
            ...
            output_base = (
                self.save_root if self.save_root is not None else (CODE_DIR / "Output")
            )
    ...
    def run_complete_pipeline(
        self, dataset_path: str, algorithm: str = "3d", model_path: str | None = None
    ) -> Dict[str, bool]:
    ...
    def run_concise_pipeline(
        self, dataset_path: str, algorithm: str = "3d"
    ) -> Dict[str, bool]:
    ...
    def interactive_mode(self) -> None:
        ...
        while True:
            ...
            if choice == "0":
                ...
                self.match_backend = "da3"
                self.run_complete_pipeline(self.default_dataset, "3d")
            elif choice == "2":
                self.match_backend = "da3"
                self.run_concise_pipeline(self.default_dataset, "3d")
            elif choice == "3":
                dataset_name = input(
                ...
                res = self.run_reconstruction(self.default_dataset, backend="da3")
                print(
                ...
                # 从da3_cache查找predictions.npz（da3 不产 GLB，以 predictions.npz 存在为准）
                recon_default = None
                cache_default = None
                cache_dir = output_dir / "da3_cache"
                candidate = cache_dir / "predictions.npz"
                if candidate.exists():
                ...
                pts_src = (
                    input("点云来源 (predictions/glb) [默认 predictions]: ").strip()
                    or "predictions"
                )
                try:
                ...
                open_browser = not (
                    (input("启动后自动打开浏览器? (Y/n): ").strip().lower() or "y")
                    == "n"
                )
                ...
                viewer_run(
                    ...
                    reconstruction=str(recon_default) if recon_default else None,
                    image_dir=str(img_default),
                    detection_dir=str(det_default),
                    cache_dir=str(cache_default) if cache_default else None,
...
def main() -> None:
    ...
    pre.add_argument(
        ...
        type=str,
        default=None,
        help="YAML 配置文件路径（默认 code/config.yaml，如存在）",
    )
    ...
    parser.add_argument(
        "--save_root",
        type=str,
        default=yaml_main.get("save_root", "Output"),
    ...
    parser.add_argument(
        ...
        default=yaml_recon.get("backend", "da3"),
        choices=["da3"],
        help="SKU匹配 3D 后端 (da3)",
    )
    parser.add_argument(
        "--parallel_refs",
        type=int,
        default=1,
        help="batch_all_refs 并行线程数（>1 时启用线程池，推荐 da3 后端）",
    )
    ...
    parser.add_argument(
        "--viewer-port", type=int, default=8080, help="viewer: 端口（默认8080）"
    )
    parser.add_argument(
        "--no-viewer-open",
        action="store_true",
    ...
    parser.add_argument(
        "--viewer-force-rebuild", action="store_true", help="viewer: 强制重建缓存"
    )
    ...
    # 统一日志
    save_root_path = (
        Path(args.save_root).expanduser().resolve()
        if args.save_root
    ...
    if args.mode == "interactive":
        ...
        app.run_improved_sku_analysis(
            args.dataset, algorithm=args.algorithm, backend=args.match_backend
        )
        ...
        viewer_run(
            global_mapping=str(
                Path(args.viewer_global_mapping)
                if args.viewer_global_mapping
                else gm_default
            ),
            reconstruction=(
                str(Path(args.viewer_reconstruction))
                if args.viewer_reconstruction
                else (str(recon_default) if recon_default else None)
            ),
            image_dir=str(
                Path(args.viewer_image_dir) if args.viewer_image_dir else img_default
            ),
            detection_dir=str(
                Path(args.viewer_detection_dir)
                if args.viewer_detection_dir
                else det_default
            ),
            cache_dir=(
                str(Path(args.viewer_cache_dir))
                if args.viewer_cache_dir
                else (str(cache_default) if cache_default else None)
            ),
            downsample_ratio=float(args.viewer_downsample),
            points_source=(
                str(args.viewer_points_source)
                if args.viewer_points_source
                else "predictions"
            ),
