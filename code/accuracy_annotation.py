...
class AccuracyAnnotator:
        ...
        try:
            ...
            for pair_name, expected_target in expected_pairs:
                ...
                if target_section and target_section["matches"]:
                    self.system_results[pair_name] = target_section["matches"]

        except Exception as e:
            print(f"加载系统结果失败: {e}")
            raise

    def extract_image_pair_from_system_path(self, result_path: str) -> str:
        """
        从系统结果文件路径推断图片对信息

        Args:
            result_path: 系统结果文件路径
        ...
        # 尝试从路径中提取图片对信息
        # 这里可以根据实际的文件组织结构进行调整
        if "output_3dmapping_da3/0" in result_path:
    ...
    def calculate_metrics(self, image_pair: str) -> Dict:
        ...
        if image_pair not in self.system_results:
            print(f"警告: 未找到图片对 {image_pair} 的系统结果")
            return {}

        # 获取人工标注和系统结果
        gt_matches = self.ground_truth[image_pair]
        system_matches = self.system_results[image_pair]

        # 转换为集合格式进行比较
        gt_set = {(match["reference_id"], match["target_id"]) for match in gt_matches}
        system_set = {
            (match["reference_id"], match["target_id"]) for match in system_matches
        }
        ...
        true_positives = len(gt_set & system_set)  # 人工标注存在且系统也预测的匹配
        false_negatives = len(
            gt_set - system_set
        )  # 人工标注存在但系统未预测的匹配（遗漏）
        ...
        # 找到系统和人工标注都有映射的reference ID
        common_ref_ids = set(gt_mapping.keys()) & set(system_mapping.keys())
        ...
        for ref_id in common_ref_ids:
            if gt_mapping[ref_id] == system_mapping[ref_id]:
                ref_mapping_correct += 1
            else:
                # 记录错误映射
                wrong_mappings.append(
                    (ref_id, system_mapping[ref_id], gt_mapping[ref_id])
                )

        # 重新计算额外预测和遗漏，排除错误映射的情况
        system_extra_matches_filtered = []
        missed_matches_filtered = []

        for match in system_set - gt_set:
            ...
            if ref_id in gt_mapping and gt_mapping[ref_id] != target_id:
                ...
                system_extra_matches_filtered.append(match)

        for match in gt_set - system_set:
            ref_id, target_id = match
            # 如果这个ref_id在系统中也存在但映射不同，则归类为错误映射，不算遗漏
            if ref_id in system_mapping and system_mapping[ref_id] != target_id:
                continue  # 已经在wrong_mappings中记录
            else:
        ...
        system_effectiveness = (
            true_positives / len(system_set) if len(system_set) > 0 else 0
        )

        # 计算基于reference ID映射的准确率
        ref_mapping_precision = (
            ref_mapping_correct / ref_mapping_total if ref_mapping_total > 0 else 0
        )

        metrics = {
            "image_pair": image_pair,
            "ground_truth_matches": len(gt_matches),
            "system_matches": len(system_matches),
            ...
            "correct_matches": list(gt_set & system_set),
            "missed_matches": missed_matches_filtered,  # 使用过滤后的遗漏匹配
            "system_extra_matches": system_extra_matches_filtered,  # 使用过滤后的额外预测
    ...
    def generate_report(self, output_path: str = None) -> str:
        ...
        for image_pair in self.ground_truth.keys():
            if image_pair in self.system_results:
        ...
        if not all_metrics:
            ...
            total_system = sum(m["system_matches"] for m in all_metrics)
            total_tp = sum(m["true_positives"] for m in all_metrics)
            total_fn = sum(m["false_negatives"] for m in all_metrics)
            total_extra = sum(len(m["system_extra_matches"]) for m in all_metrics)
            total_wrong_mappings = sum(len(m["wrong_mappings"]) for m in all_metrics)

            # 计算总体reference ID映射指标
            total_ref_mapping_correct = sum(
                m["ref_mapping_correct"] for m in all_metrics
            )
            ...
            overall_effectiveness = total_tp / total_system if total_system > 0 else 0
            overall_ref_mapping_precision = (
                total_ref_mapping_correct / total_ref_mapping_total
                if total_ref_mapping_total > 0
                else 0
            )
            ...
            report_lines.extend(
                [
                    ...
                    "Reference ID映射准确率: 当系统和人工标注都对同一ref ID有映射时，映射目标是否一致",
                    "",
                ]
            )
            ...
            for metrics in all_metrics:
                if metrics["image_pair"]:  # 确保有图片对数据
                    report_lines.extend(
                        [f"图片对 {metrics['image_pair']} 详细分析:", "-" * 50]
                    )

                    # 系统额外预测的匹配
                    if metrics["system_extra_matches"]:
                        report_lines.extend(
                            [
                                "系统额外检出对 (人工标注中不存在):",
                                f"  {metrics['system_extra_matches']}",
                                "",
                            ]
                        )
                    ...
                    if metrics["wrong_mappings"]:
                        ...
                        for ref_id, system_target, gt_target in metrics[
                            "wrong_mappings"
                        ]:
                            report_lines.append(
                                f"  ref_{ref_id}: 系统预测→{system_target}, 人工标注→{gt_target}"
                            )
                        report_lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--benchmark-csv", required=True, help="人工标注数据集CSV文件路径"
    )
    parser.add_argument(
        "--system-result", required=True, help="系统输出的匹配结果文件路径"
    )
    parser.add_argument("--dataset-filter", default=None, help="数据集过滤器")
    ...
    if not os.path.exists(args.system_result):
        print(f"错误: 系统结果文件不存在: {args.system_result}")
        return
    ...
    try:
        ...
        annotator.load_system_results(args.system_result)
        ...
        if args.output:
            ...
            annotator.generate_report(default_output)
