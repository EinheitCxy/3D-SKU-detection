...
`--mode ground-stack-area` 读取既有 `detections_results/` 和 `dedup_detections/global_mapping.json`，使用一个已知正面宽高的检测框作为标定锚点，只将锚点帧内每个物理 `global_id` 的有效 bbox 换算为 `cm²`，再得到总 `m²`。其他帧的观测不会参与面积选择；缺少锚点帧观测的 global ID 会被拒绝，以避免相机运动造成的跨帧尺度误差。多个 `skus` 组使用同一稳定对象索引参与匹配、去重与计量。必须显式提供锚点帧号、对象索引、宽度和高度；没有任何隐式锚点默认值。
