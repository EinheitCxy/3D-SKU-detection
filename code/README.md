...
逐源帧 SAM3 mask cache 是内部 utility：它以不可变、完整验证的 bundle 持久化 source masks，包含 source image、完整有序 prompts、caller-supplied opaque `checkpoint_sha256` 及 code/runtime/predict contract 的 provenance。它不验证 checkpoint 文件，也不验证加载时的一致性；该 TOCTOU contract 由 Task 3 的 SAM3 helper 在模型加载边界执行：helper 会在加载前后对 checkpoint bytes 做 SHA-256 验证，in-process model cache 只以 checkpoint digest、normalized device 与固定 `predict_inst` inference-contract fingerprint 为键。公开 `ground-stack-area` 命令在 Task 4 前不会读取这个 persistent mask cache；cache 目录不构成稳定用户接口。该 utility 绝不提供 bbox fallback 或 partial total，空/无效 mask 必须在集成后的正式阶段保持 rejected/null 语义。
