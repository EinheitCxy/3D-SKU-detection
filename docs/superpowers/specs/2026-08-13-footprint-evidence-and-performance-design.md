...
A hit is valid only when manifest, payload digest, source image, ordered detections, checkpoint digest, code/runtime fingerprints, count, bool dtype, shape, per-mask digest, pixel count, and bbox clipping exactly match. Cache producer validation and hit validation must call the same canonical `clip_mask_to_bbox` primitive as SAM3 output postprocessing, including its boundary-pixel behavior for fully out-of-image prompts; they may not carry a second clipping implementation. Any semantic cache-schema revision is a key invalidator. Any mismatch is an audited miss, never a bbox fallback.
...
Formal artifacts use immutable generations at save_root/dataset/ground_stack_footprint/runs/run_id containing the report, GeoJSON, PNG, and a complete manifest. After all files and the generation directory are fsynced, a single CURRENT manifest/pointer is atomically replaced. Generation construction can proceed concurrently, but a fixed output-root `flock` must cover CURRENT replacement and resolution of the publishing call's own expected run ID. Public artifact readers take the corresponding shared lock before resolving CURRENT; a reader therefore gets one complete old generation or one complete new generation, never a three-file mixture, and a publisher never returns another call's report path. A failure before the CURRENT replace leaves the prior complete generation or no generation visible. A successful CURRENT replace is the logical publication point: a subsequent parent-directory fsync failure is reported as a durability warning but does not falsely report publication failure or attempt to roll a complete visible generation back. Tests inject each pre-replace boundary, the post-replace fsync boundary, and deterministic two-publisher interleavings.
...
A shadow mask robustness diagnostic runs documented one-pixel source-space erosion and dilation through the same source-to-processed affine warp, point, 5-mm voxel/component/OBB, precision union, and all-ID operations. It retains both actual source masks and processed masks in the evidence observation, labels their pixel counts separately, and must not approximate source-space morphology on the processed grid. It reports area interval and rejection transitions but never substitutes a perturbed mask for the formal mask. This diagnostic is independent of optional K/E/depth availability; camera absence can make reprojection unavailable while mask robustness remains available.
...
Primary report fields remain stable. New additive sections are performance with monotonic `validation_and_io`, `sam3_source_masks`, `select_support_plane`, `per_id_obb_union`, `shadow_evidence`, and `artifact_creation` stage seconds plus a pre-publication total and RANSAC diagnostics; sam3_mask_cache with per-frame provenance and exact `cache_events`; and evidence with mode, camera contract, per-global-ID observations, numerical sensitivity, empirical sensitivity, and source-mask robustness. A stage never entered is `null`, not a fabricated zero; timing is diagnostic only and cannot affect acceptance.
