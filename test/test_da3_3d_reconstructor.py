...
def test_da3_reconstructor_uses_output_parent_for_runner_partial_and_publish(
    ...
    def fake_runner(cmd, **_):
        assert cmd[cmd.index("--process_res") + 1] == "504"
    ...
    assert reconstructor._active_cache_dir is None
...
@pytest.mark.parametrize("size,expected", [((1080,1920),(280,504)), ((1920,1080),(504,280)), ((1600,1200),(504,378)), ((1200,1600),(378,504))])
def test_da3_default_preprocessing_preserves_orientation_and_full_frame(size, expected):
    from PIL import Image
    from src import da3_runner
    from depth_anything_3.utils.io.input_processor import InputProcessor
    args = da3_runner._parse_args(["--input_dir", "unused", "--output_npz", "unused"])
    assert args.process_res == 504
    da3_runner._validate_image_orientations([size, size])
    batch, _, _ = InputProcessor()([Image.new("RGB", size)], process_res=args.process_res, process_res_method=da3_runner.PREPROCESS_METHOD, sequential=True)
    assert tuple(batch.shape[-2:]) == expected[::-1]
    affine = da3_runner._source_to_processed_affines([size], expected[1], expected[0], args.process_res)[0]
    # Pixel cell boundaries must map to the full output, not a central crop.
    assert np.allclose(affine @ [-.5,-.5,1], [-.5,-.5], atol=1e-5)
    assert np.allclose(affine @ [size[0]-.5,size[1]-.5,1], np.array(expected)-.5, atol=1e-4)


def test_da3_rejects_mixed_orientations():
    from src import da3_runner
    with pytest.raises(ValueError, match="横屏.*竖屏"):
        da3_runner._validate_image_orientations([(1080,1920), (1920,1080)])


def test_da3_rejects_same_orientation_with_different_grids():
    from src import da3_runner
    with pytest.raises(ValueError, match="不允许自动裁切"):
        da3_runner._validate_image_orientations([(1920,1080),(1600,1200)])


def test_cache_reuse_rejects_old_resolution_and_cropped_batches(tmp_path):
    from main import _is_reusable_da3_cache
    path = tmp_path / "predictions.npz"
    def write(resolution, sizes):
        np.savez(path, cache_schema_version=np.asarray(3), is_metric=np.asarray(1),
                 preprocess_resolution=np.asarray(resolution),
                 preprocess_method=np.asarray("upper_bound_resize"),
                 source_image_sizes=np.asarray(sizes))
    write(504, [(1920,1080),(1280,720)])
    assert _is_reusable_da3_cache(path)
    write(896, [(1920,1080)])
    assert not _is_reusable_da3_cache(path)
    write(504, [(1920,1080),(1600,1200)])
    assert not _is_reusable_da3_cache(path)
