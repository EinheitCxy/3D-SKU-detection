...
def test_personalcare_classification_rejects_nonpublished_subprocess_results(
    ...
    with pytest.raises(RuntimeError):
        main.SKUDetectionMain().run_personalcare_classification(
...
def test_personalcare_classification_rejects_unbound_publication_payloads(
    ...
    with pytest.raises(RuntimeError):
        app.run_personalcare_classification(str(dataset))
...
def test_classification_failure_stops_before_dedup(
    ...
    monkeypatch.setattr(
        app,
        "run_personalcare_classification",
        lambda _path: (_ for _ in ()).throw(RuntimeError("model failed")),
    )
    monkeypatch.setattr(
        app,
        "run_sku_matching",
...
@pytest.mark.parametrize("resolution,reuse", [(504, True), (896, False), (None, False)])
def test_da3_pipeline_reuses_only_metric_schema_v3_predictions_cache(
    monkeypatch, tmp_path: Path, resolution, reuse
) -> None:
    dataset = tmp_path / "datasets" / "sample"
    output_root = tmp_path / "runtime-output"
    ...
    np.savez_compressed(
        ...
        preprocess_method=np.asarray("upper_bound_resize"),
        source_image_sizes=np.asarray([[504, 504]], dtype=np.int32),
        **({"preprocess_resolution": np.asarray(resolution, dtype=np.int32)} if resolution is not None else {}),
    )
    app = main.SKUDetectionMain()
    app.save_root = output_root
    app.match_backend = "da3"
    ...
    assert calls == ([] if reuse else ["reconstruct"])
...
def test_batch_matching_serial_failure_stops_next_reference(
    monkeypatch, tmp_path: Path
) -> None:
    """A failed first reference must stop the serial batch."""
    dataset = tmp_path / "datasets" / "sample"
    detections = dataset / "detections_results"
    ...
    def fake_single(*_args, **_kwargs):
        ...
        if reference_idx == 0:
            raise RuntimeError("reference mask failed")
        return {"success": True}

    monkeypatch.setattr(app, "_run_single_matching", fake_single)

    with pytest.raises(RuntimeError, match="reference mask failed"):
        app.run_sku_matching(str(dataset), batch_all_refs=True, parallel_refs=1)
    assert calls == [0]


def test_serial_batch_matching_propagates_exception_without_continuing(
    monkeypatch, tmp_path: Path
) -> None:
    """A direct serial exception aborts subsequent references."""
    dataset = tmp_path / "datasets" / "sample"
    detections = dataset / "detections_results"
    detections.mkdir(parents=True)
    app = main.SKUDetectionMain()
    ...
    with pytest.raises(RuntimeError, match="serial worker exploded"):
        app.run_sku_matching(str(dataset), batch_all_refs=True, parallel_refs=1)
    assert completed == [0]


def test_parallel_batch_matching_propagates_future_exceptions(
    monkeypatch, tmp_path: Path
) -> None:
    """A failed future propagates after executor cleanup."""
    dataset = tmp_path / "datasets" / "sample"
    detections = dataset / "detections_results"
    detections.mkdir(parents=True)
    app = main.SKUDetectionMain()
    ...
    with pytest.raises(RuntimeError, match="(reference mask failed|worker exploded)"):
        app.run_sku_matching(str(dataset), batch_all_refs=True, parallel_refs=2)
...
def test_complete_pipeline_matching_failure_stops_later_publication(
    ...
    monkeypatch.setattr(
        app,
        "run_sku_matching",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("matching failed")),
    )
    monkeypatch.setattr(
        app, "run_improved_sku_analysis", lambda *_args, **_kwargs: pytest.fail("analysis after matching failure")
    )
    monkeypatch.setattr(
        app, "run_dedup_sequence", lambda *_args, **_kwargs: pytest.fail("dedup after matching failure")
    )
    monkeypatch.setattr(
        app,
        "run_personalcare_classification",
    ...
    monkeypatch.setattr(
        app, "run_accuracy_evaluation", lambda *_args, **_kwargs: pytest.fail("accuracy after matching failure")
    )

    with pytest.raises(RuntimeError, match="matching failed"):
        app.run_complete_pipeline(str(dataset), algorithm="point_tracking")
...
def test_root_parallel_refs_publish_real_complete_frame_cache(
    ...
    for frame_id, frame in enumerate(frames):
        ...
        request = sam3_utils._processed_frame_request(
            cache_root=default_sam3_mask_cache_root(output_root / dataset.name),
