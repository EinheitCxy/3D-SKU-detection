...
import { applyVisibilityUpdates, buildPointRangeLookup, buildVisibilityDelta, firstVisiblePointGlobalId, globalIdForPointIndex, visiblePointRanges } from "../../modules/viewer_web/src/point-picking";
import type { ObjectIndex } from "../../modules/viewer_web/src/contracts";
...
describe("point-only picking", () => {
  ...
  it("does not pick through an unassigned visible foreground point", () => {
    const lookup = buildPointRangeLookup(objectIndexWithRanges({ "7": [[1, 2]] }));
    expect(firstVisiblePointGlobalId([0, 1], lookup, new Set(["7"]))).toBeNull();
  });
});
